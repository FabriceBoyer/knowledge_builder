...
export function WordsPage() {
  ...
  const displayed = state.senses.filter((sense) => (!groupFilter || sense.groups?.includes(groupFilter)) && (!labelFilter || sense.labels?.includes(labelFilter)))
  ...
  return <div className="page-container">
    ...
    <section className="palette-section">
      ...
      {state.senses.length ? <><div className="palette-filters"><label>Group<select value={groupFilter} onChange={(event) => setGroupFilter(event.target.value)}><option value="">All groups</option>{groups.map((group) => <option key={group}>{group}</option>)}</select></label><label>Label<select value={labelFilter} onChange={(event) => setLabelFilter(event.target.value)}><option value="">All labels</option>{labels.map((label) => <option key={label}>{label}</option>)}</select></label>{(groupFilter || labelFilter) && <button className="secondary-button" onClick={() => { setGroupFilter(''); setLabelFilter('') }}>Clear filters</button>}</div>
