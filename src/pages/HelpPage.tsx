...
export function HelpPage() {
  ...
  return <div className="page-container help-page">
    ...
    <div className="help-layout"><nav><a href="#senses">1. Collect senses</a><a href="#graphs">2. Compose concepts</a><a href="#articles">3. Map an article</a><a href="#storage">4. Your data</a></nav><div className="help-content">
      ...
      <section id="storage"><div className="help-icon"><HardDrive /></div><div><span>LOCAL FIRST</span><h2>Your work stays with you</h2><p>Every sense, graph position, relationship, and article annotation is immediately stored in this browser, then synchronized to PocketBase in the background. Refreshing or closing the tab will not erase the workspace, and an outage never blocks local editing.</p><div className="privacy"><ShieldCheck /><span><strong>Private by default.</strong><br />A random browser identity owns one isolated cloud workspace. The header cloud icon shows its sync state.</span></div><button className="danger-button" onClick={() => window.confirm('Erase the entire Lexigraph workspace locally and in PocketBase?') && clearWorkspace()}>Reset workspace</button></div></section>
    </div></div>
  </div>
}
